...
<script>
	...
	function changePage(page) {
		var url;
		if (categoryNo) {
			url = "/ticket/admin/qna/listQnasByCategory?categoryNo=" + categoryNo + "&page=" + page;
		} else {
			url = "/ticket/admin/qna/listQnas?page=" + page;
		}
	    $.ajax({
	        type: 'GET',
	        url: url,
	...
	function updatePageButtons(currentPage) {
	    ...
	    if (currentPage <= 1) {
	        $('#qna-prev-btn').prop('disabled', true);
	    } else {
	        $('#qna-prev-btn').prop('disabled', false);
	    }
	    // 다음 페이지로 이동하는 버튼 활성화 여부 결정
	    if (currentPage >= totalPages) {
	        $('#qna-next-btn').prop('disabled', true);
	    } else {
	        $('#qna-next-btn').prop('disabled', false);
	    }
	    // 페이지 번호 버튼들 업데이트
...
<body>
	<div id="cs-board-list">
		...
		<c:forEach var="qna" items="${qnasList}">     
			<div id="cs-board-list-row">
                <div class="cs-board-list-row-box">
                    <input type="checkbox" name="selectedQnas" value="${qna.qnaNo}">
                </div>
				<div class="cs-board-list-row-box">
					${qna.category}
				</div>
				<div class="cs-board-list-row-box">
            		<a href="#" class="qna-link" data-qna-id="${qna.qnaNo}">${qna.title}</a>
        		</div>
				<div class="cs-board-list-row-box">
					${qna.regDate}
				</div>
	...
	<div id="cs-board-page">
		<!-- 이전 페이지로 이동하는 버튼 -->
		<button id="qna-prev-btn" class="move-buttons" onclick="changePage(${currentPage - 1})">&lt;</button>
		
		<!-- 페이지 번호 버튼들 -->
		<c:forEach var="pageNum" begin="1" end="${totalPages}">
		    <button id="qna-page-buttons" class="page-buttons" onclick="changePage(${pageNum})" value="${pageNum}">${pageNum}</button>
		</c:forEach>
		
		<!-- 다음 페이지로 이동하는 버튼 -->
		<button id="qna-next-btn" class="move-buttons" onclick="changePage(${currentPage + 1})">&gt;</button>
	</div>
</body>
</html>
